...
def random_number_at_least(duration):
    return duration + random.expovariate(10)

def sleep_at_least(duration):
    sleep_for = duration + random.expovariate(10)
    print(f"Sleeping: {sleep_for}")
    time.sleep(sleep_for)

def sleep_a_tick():
    time.sleep(0.6 + random.expovariate(10))

def tick_duration():
    delay = 0.6 + random.expovariate(10)
    return delay
